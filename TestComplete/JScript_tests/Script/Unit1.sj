function Main()
{
   if(Project.Variables.Input != null){
    ...
    var driver = DDT.CSVDriver("D:\\TestProScripts\\TestComplete\\JScript_tests\\Testdata\\testdata_orders.csv");
  }
  
  var process = TestedApps.Orders.Run();
  var i = 0;
...
function newOrder(process, driver){
  ...
  var w = Sys.Process("Orders").WinFormsObject("MainForm");
  w.MainMenu.Click("Orders|New order...")
  
  var orderform = Sys.Process("Orders").WinFormsObject("OrderForm").WinFormsObject("Group")
  
  var product = driver.Value("product");
  var qty = driver.Value("quantity");
  ...
  var price = {"MyMoney":100, "FamilyAlbum":80, "ScreenSaver":20}
  ...
  var discount = {"MyMoney":8, "FamilyAlbum":15, "ScreenSaver":10}
...
function CloseOrders(process){
  ...
  var msgBox = process.Window('#32770', 'Confirmation');
  if (msgBox.Exists)
  {
